...
class BODY(object):
    ...
    def count_active_voxels(self, structure):
        ...
        hs_outofphase = np.sum( structure == 5 )
        vs_outofphase = np.sum( structure == 6 )
        return hs + vs + hs_outofphase + vs_outofphase

class FIXED_BODY(BODY):
    def __init__(self, fixed_body=None, fixed_body_path=None, args=None):
        ...
        if fixed_body is not None:
            for body in fixed_body:
                if body == "biped":
                    ...
                    structure = triped
                    name = "triped"
                elif body in ['centralized', 'decentralized', 'noobs']:
                    if "Bridge" in args.task:
                        to_open_name = "bridge_bodies_"
                    else:
                        to_open_name = "walker_bodies_"
                    to_open_name += str(args.bounding_box[0]) + "x" + str(args.bounding_box[1]) + "_" + body + ".npy"
                    structures = np.load(to_open_name)
                    structure = structures[args.repetition[0]-1]
                    name = body
                else:
...
class BASIC_BODY(BODY):
    def __init__(self, args):
        ...
        # TODO: empty body yerine random body ile baslanabilir, assagida yazilmis zaten random body
        self.mutate()
