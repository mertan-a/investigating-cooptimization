...
class ObservationWrapper(gym.Wrapper):
    ...
    def get_volumes_from_pos(self):
        ...
        for idx, corners in enumerate(structure_corners):
            ...
            volumes[x,y] = (vol/0.01) - 1.0 # 0.01 is the area of a voxel at rest
            mask[x,y] = 1
        return volumes, mask
    ...
    def get_velocities_from_pos(self):
        ...
        for idx, corners in enumerate(structure_corners):
            if corners == None:
                continue
            x, y = self.two_d_idx_of(idx)
            vx, vy = self.voxel_velocity([v[0] for v in corners], [v[1] for v in corners])
            velocities[x,y,0] = vx
            velocities[x,y,1] = vy
            mask[x,y] = 1
...
class LocalObservationWrapper(ObservationWrapper):
    ...
    def __init__(self, env, **kwargs):
        ...
        obs_len += self.nr_voxel_in_neighborhood*2 # velocity
        obs_len += 1 # time
...
class GlobalObservationWrapper(ObservationWrapper):
    ...
    def __init__(self, env, **kwargs):
        ...
        obs_len += 2*self.robot_structure.size # velocity
        obs_len += 1 # time
...
if __name__ == '__main__':
    ...
    kwargs = {'observe_structure': True,
            ...
            'observe_time': True,
            'observe_time_interval': 20,
              'observation_range': 2}

    np.float = float
    env = gym.make('Walker-v0', body=worm, connections=get_full_connectivity(worm))
